...
impl Hash for DaysDelta {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        match self {
            DaysDelta::Forwards(d) | DaysDelta::Backwards(d) if *d == Days::new(0) => {
                Days::new(0).hash(state)
            }
...
impl NaiveDate {
    ...
    pub fn checked_add_days(self, days: Days) -> Option<Self> {
        ...
        let cycle =
            internals::yo_to_cycle(u32::try_from(year_mod_400).unwrap(), self.of().ordinal());
        let cycle = cycle.checked_add(days.0).unwrap();
        let (cycle_div_400y, cycle) = div_mod_floor(cycle, 146_097);

        year_div_400 += i64::from(cycle_div_400y);

        let (year_mod_400, ordinal) = internals::cycle_to_yo(cycle);
        let flags = YearFlags::from_year_mod_400(i32::try_from(year_mod_400).unwrap());

        let year = i32::try_from(year_div_400 * 400 + i64::from(year_mod_400)).unwrap();
        NaiveDate::from_of(year, Of::new(ordinal, flags))
    }
    ...
    pub fn checked_sub_days(self, days: Days) -> Option<Self> {
        ...
        let cycle = cycle.checked_sub(i64::from(days.0)).unwrap();
        let (cycle_div_400y, cycle) = div_mod_floor(cycle, 146_097);

        year_div_400 += cycle_div_400y;

        let (year_mod_400, ordinal) = internals::cycle_to_yo(u32::try_from(cycle).unwrap());
        let flags = YearFlags::from_year_mod_400(i32::try_from(year_mod_400).unwrap());

        NaiveDate::from_of(
            i32::try_from(year_div_400 * 400 + i64::from(year_mod_400)).unwrap(),
    ...
    pub fn days_since(self, rhs: NaiveDate) -> DaysDelta {
        ...
        if self >= rhs {
            DaysDelta::Forwards(Days::new(u32::try_from(diff).unwrap()))
        } else {
            DaysDelta::Backwards(Days::new(u32::try_from(diff.abs()).unwrap()))
        }
    }
